        ...
        <table>
            <tr>
                <td >
                    ...
                    <button id="btn_confirm_transmittal1" type="button">Confirm</button>
                </td>
            </tr>
            <tr>
        ...
        <table>
            <tr>
                <td >
                    Notes:<br>
                    <textarea style="width:600px;height:100px"></textarea><br>
                    <button id="btn_confirm_transmittal2" type="button">Confirm</button>
                </td>
            </tr>
            <tr>
                <td style="text-align: center" class="small">
...
<div style="border:5px rgb(0,128,0) solid">
    ...
    <table width="100%">
        ...
        <tr>
            <td>
                <textarea style="width:600px;height:100px"></textarea><br>
                <button id="btn_confirm_invoice" type="button">Confirm</button>
            </td>
        </tr>
        <tr>
...
<div id="dialog_manual_invoice_viewer">
    ...
    <button id="btn_confirmed_manual_invoice" type="button">Confirm</button>
</div>
...
<script type="text/javascript">
    $.extend({
        submitConfirmData : function(type,contents,dialog_elm){
            var billing_month =$('#month').val()
                ...
                $.ajax({
                    type: "POST",
                    url: '../billing/invoice_generate_contract_action',
                    data: {'action' : 'confirm','type': type,'contents': contents,'billing_month':billing_month,'billing_year':billing_year,'customer_id':customer_id},
                    success: function(msg){
                        $('#'+dialog_elm).dialog('close');
                        $.updateStatusMultipleButtonConfirm();
                    }
                });
        ...
        ,updateStatusMultipleButtonConfirm : function(){
            var billing_month =$('#month').val()
                ,billing_year =$('#year').val()
                ,customer_id = $('#cmb_customers').val();
            ...
            $.ajax({
                type: "POST",
                url: '../billing/invoice_generate_contract_action',
                data: {'action' : 'get-invoice-data','type': '1,2,3,4','billing_month':billing_month,'billing_year':billing_year,'customer_id':customer_id},
                async : true,
                success: function(data){
                    var is_confirm_transmittal1 = 0
                        ,is_confirm_transmittal2 = 0
                        ,is_confirm_invoice = 0
                        ,is_confirm_manual_invoice = 0
                        ,proceed = 0
                        ,items_confirm = [];
                    ...
                    if ( data.total > 0 ) {
                        ...
                        if ( typeof data.value['type_1'] != 'undefined' ) {
                            is_confirm_transmittal1 = 0;
                        }else {
                            is_confirm_transmittal1 = 1;
                            items_confirm.push('Transmittal 1');
                        }
                        ...
                        if ( typeof data.value['type_2'] != 'undefined' ) {
                            ...
                            is_confirm_transmittal2 = 1;
                            items_confirm.push('Transmittal 2');
                        }
                        ...
                        if ( typeof data.value['type_3'] != 'undefined' ) {
                            ...
                            is_confirm_invoice = 1;
                            items_confirm.push('Invoice');
                        }

                        if ( typeof data.value['type_4'] != 'undefined' ) {
                            is_confirm_manual_invoice = 0;
                        }else {
                            is_confirm_manual_invoice = 1;
                            items_confirm.push('Manual Invoice');
                        }

                        if (items_confirm.length > 0) {
                            $('#btn_multiple_confirm').show();
                            $('#fs-upload-manual').show();
                        }else {
                            $('#btn_multiple_confirm').hide();
        ...
        ,multipleConfirm : function(){
            ...
            $.ajax({
                ...
                success: function(data){
                    ...
                    if (proceed === 1) {
                        var confirm_msg = '';
                        if ( items_confirm.length === 3 || items_confirm.length === 0) {
                            confirm_msg = 'Are you sure you want to confirm all?'
                        }else {
                            confirm_msg = 'Are you sure you want to confirm ' + items_confirm.join('and ') + '? ';
                        }
                        ...
                        if (r==true){
                            ...
                            var contents1= '';
                            if ( is_confirm_transmittal1 === 1 ) {
                                contents1= $('#dialog_transmittal_1').html();
                                contents1 = contents1.replace(/<button id="btn_confirm_transmittal1" type="button">Confirm<\/button>/gi,'');
                            }

                            var contents2= '';
                            if ( is_confirm_transmittal2 === 1 ) {
                                contents2= $('#dialog_transmittal_2').html();
                                contents2 = contents2.replace(/<button id="btn_confirm_transmittal2" type="button">Confirm<\/button>/gi,'');
                            }

                            var contents3= '';
                            if ( is_confirm_invoice === 1 ) {
                                contents3= $('#dialog_invoice').html();
                                contents3 = contents3.replace(/<button id="btn_confirm_invoice" type="button">Confirm<\/button>/gi,'');
                            }

                            var parameters = {};
                            parameters['action'] = 'confirm-multiple';
                            parameters['is_confirm_transmittal1'] = is_confirm_transmittal1;
                            parameters['is_confirm_transmittal2'] = is_confirm_transmittal2;
                            parameters['is_confirm_invoice'] = is_confirm_invoice;
                            parameters['is_confirm_manual_invoice'] = is_confirm_manual_invoice;
                            parameters['contents1'] = contents1;
                            parameters['contents2'] = contents2;
                            parameters['contents3'] = contents3;
        ...
        ,generateInvoicesPDF : function(){
            ...
            parameters += '&customer_id=' + customer_id;
            parameters += '&email=0';
            $.download('../billing/file_and_email_invoice_generate_contract',parameters);
        }
            ...
            $.ajax({
                ...
                success: function(data){
                    if (typeof data.value.date_created != 'undefined' ){
                        ...
                        var contents ='<button id="btn_multiple_confirm" type="button">Confirm All</button>' +
                            '<button id="btn_send_email" type="button">Sent to Email</button>' +
        ...
        ,sendToEmail : function() {
            ...
            $.ajax({
                type: "POST",
                url: '../billing/file_and_email_invoice_generate_contract',
                data: parameters,
                success: function(msg){
        ...
        ,submitManualInvoiceFile : function(){
            var path = '../billing/invoice_generate_contract_submit_manual_invoice';
        ...
        ,getManualInvoiceFilePath : function(){
            ...
            $.ajax({
                ...
                success: function(data){
                    ...
                    var siteurl = document.location.protocol+'//'+document.location.hostname+baseurl+'/billing/invoice_generate_contract_get_manual_invoice_file?filepath=';
    ...
    $(document).ready(function(){
        ...
        $('#btn_confirm_transmittal1').unbind().live('click',function(){
            var contents= $('#dialog_transmittal_1').html();
            contents = contents.replace(/<button id="btn_confirm_transmittal1" type="button">Confirm<\/button>/gi,'');
            $.submitConfirmData(1,contents,'dialog_transmittal_1');
        });

        $('#btn_confirm_transmittal2').unbind().live('click',function(){
            var contents= $('#dialog_transmittal_2').html();
            contents = contents.replace(/<button id="btn_confirm_transmittal2" type="button">Confirm<\/button>/gi,'');
            $.submitConfirmData(2,contents,'dialog_transmittal_2');
        });

        $('#btn_confirm_invoice').unbind().live('click',function(){
            var contents= $('#dialog_invoice').html();
            contents = contents.replace(/<button id="btn_confirm_invoice" type="button">Confirm<\/button>/gi,'');
            $.submitConfirmData(3,contents,'dialog_invoice');
        });

        $('#btn_multiple_confirm').unbind().live('click',function(){
            $.multipleConfirm();
        });
        ...
        $('#btn_confirmed_manual_invoice').unbind().live('click',function(e){
            e.preventDefault();
            $.submitConfirmData(4,$('#hid_manual_invoice_filepath').val(),'dialog_manual_invoice_viewer');
        });
