...
<div class="span12">
    ...
    <div class="row-fluid">
		...
		<div class="span3 input-append"><input type="text" id="datepicker" name="date" value="<?=$date?>" class="input-medium"><button class="btn btn-primary" id="submit_filter">Retrieve</button></div>
    </div>
    <br>
...
$.extend({
    ...
    insertData : function(){
        ...
        $.post('../trading_dandap/dap_plant_override_action',$('#frm_data').serialize(),
        function(data){
            $('#alert_msg').html(data);
        });	
        return false;
    ...
    loadData : function(){
        ...
        $.post('../trading_dandap/dap_plant_override_load',{resource_id:$('#resource').val(),date:$('#datepicker').val(),type:'DAP'},
        function(data){
            data = $.parseJSON(data);
...
$('#frm1').submit(function(e){
    ...
    var options = {target:'#result',
        ...
        success: function(data) {
            ...
            var obj = data.value;
            var resource_html;
                resource_html += '<option>'+obj.resource+'</option>'; 
            
            $('#resource').html(resource_html);
            $('#plant option[value="'+obj.plant_name+'"]').attr('selected','selected');
