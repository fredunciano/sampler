    ...
    <section id="global">
        ...
        <div class="row-fluid">
            ...
            <div class="span10 input-append input-prepend">
                <input id="sdate" name="sdate" value="<?=$sdate?>" type="text" class="input-small"><span class="add-on">to</span>
                <input id="edate" name="edate" value="<?=$edate?>" type="text" class="input-small">
...
<script>
    $.extend({
        generateFile : function(filename){
            $.download('../market_data/generate_file','file='+filename+'&dir=significant_variations_report&type=pdf');
        }
    })
</script>
<script>
...
$('.btn').unbind('click').bind('click',function(e){
    ...
    $.post('../market_data/file_ret_svr_action',{sdate:$('#sdate').val(),edate:$('#edate').val()},
        function(data){
            if (data.total < 1) {
