...
<script>
    $.extend({
        generateFile : function(filename){
            $.download('../market_data/generate_file','file='+filename+'&dir=discrepancy_dispatch_monitoring_report&type=pdf');
        }
    })
</script>
<script>
...
$('.btn').unbind('click').bind('click',function(e){
    ...
    $.post('../market_data/file_ret_ddmr_action',{month:$('#month').val(),year:$('#year').val(),
        region:$('#region').val()},
        function(data){
            if (data.total < 1) {
