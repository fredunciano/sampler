...
<script>
    $.extend({
        generateFile : function(filename){
            $.download('../market_data/generate_file','file='+filename+'&dir=monthly_summary_report&type=pdf');
        }
    })
</script>
<script>
$('.btn').unbind('click').bind('click',function(e){
    ...
    $.post('../market_data/file_ret_msr_action',{year:$('#year').val()},
        function(data){
            if (data.total < 1) {
                ...
                html+= '<tr><th>Year</th><th>Month</th><th>Filename</th><th></th></tr>';
                $.each(data.value, function(i,val){
                    html+='<tr><td>'+val.year+'</td><td>'+val.month+
